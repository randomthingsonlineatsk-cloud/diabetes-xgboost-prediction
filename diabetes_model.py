# Cell 1 - Install
!pip install shap -q
# Cell 2 - Full Pipeline (dataset embedded, no internet or file needed)
...
# ===========================================================================
# STEP 1 - LOAD DATA (embedded directly in code)
# ===========================================================================
...
df_raw = pd.read_csv(io.StringIO(RAW_CSV), header=None, names=cols)

print("="*60)
print("STEP 1 - RAW DATA OVERVIEW")
print("="*60)
print(f"Shape  : {df_raw.shape[0]} rows x {df_raw.shape[1]} columns")
print(f"Classes: Non-Diabetic={sum(df_raw.Outcome==0)}  |  Diabetic={sum(df_raw.Outcome==1)}")
print("\nFirst 5 rows:")
...
print("\nZero (impossible) values per column:")
...
def clip_outliers(series, factor=2.5):
...
print("Outlier clipping applied (IQR x 2.5)")
...
for ax, col in zip(axes.flatten(), feat_cols):
    for outcome in [0, 1]:
        ...
        ax.hist(data, bins=25, alpha=0.6, color=colors[outcome],
                label=labels[outcome], edgecolor="white", linewidth=0.5)
    ax.set_title(col, fontweight="bold", fontsize=11, color=BLUE)
    ax.tick_params(labelsize=8)
    ax.spines[["top","right"]].set_visible(False)
axes.flatten()[-1].axis("off")
patches = [mpatches.Patch(color=colors[k], label=v) for k,v in labels.items()]
fig.legend(handles=patches, loc="lower right", bbox_to_anchor=(0.98, 0.02),
           fontsize=12, framealpha=0.9)
plt.tight_layout()
plt.savefig(f"{OUTPUT_DIR}/fig1_distributions.png", bbox_inches="tight", facecolor="white")
plt.show()
print("Figure 1 saved: Feature distributions")
...
ax.set_title("Feature Correlation Heatmap", fontsize=14, fontweight="bold",
             color=BLUE, pad=15)
plt.tight_layout()
plt.savefig(f"{OUTPUT_DIR}/fig2_correlation.png", bbox_inches="tight", facecolor="white")
plt.show()
print("Figure 2 saved: Correlation heatmap")

fig, axes = plt.subplots(2, 4, figsize=(16, 8))
fig.suptitle("Feature Box Plots by Diabetes Status", fontsize=15,
             fontweight="bold", color=BLUE, y=1.01)
for ax, col in zip(axes.flatten(), feat_cols):
    data_grp = [df[df.Outcome==0][col], df[df.Outcome==1][col]]
    ...
    ax.spines[["top","right"]].set_visible(False)
plt.tight_layout()
plt.savefig(f"{OUTPUT_DIR}/fig3_boxplots.png", bbox_inches="tight", facecolor="white")
plt.show()
print("Figure 3 saved: Box plots")
...
df_fe["BMI_Category"]     = df_fe["BMI"].apply(bmi_cat)
df_fe["Age_Group"]        = df_fe["Age"].apply(age_grp)
df_fe["Glucose_Category"] = df_fe["Glucose"].apply(gluc_cat)
...
print(f"New features: BMI_Category, Age_Group, Glucose_Category, Glucose_BMI, Age_BMI, Insulin_Glucose")
print(f"Final feature count: {df_fe.shape[1]-1}")
...
cv_pipeline = Pipeline([("scaler", StandardScaler()), ("xgb", model)])
cv_scores = cross_val_score(cv_pipeline, X_train, y_train, cv=cv,
...
with open(f"{OUTPUT_DIR}/metrics.json", "w") as f:
    json.dump({k: round(v * 100, 2) for k, v in metrics.items()}, f)
...
bars = ax1.barh(list(metrics.keys()), list(metrics.values()),
                color=[BLUE,TEAL,CORAL,GREEN,BLUE], alpha=0.85, height=0.55)
for bar, val in zip(bars, metrics.values()):
    ax1.text(bar.get_width()+0.005, bar.get_y()+bar.get_height()/2,
             f"{val:.3f}", va="center", fontsize=9, fontweight="bold")
ax1.set_xlim(0, 1.12)
ax1.set_title("Performance Metrics", fontweight="bold", color=BLUE, fontsize=12)
ax1.spines[["top","right"]].set_visible(False)
ax1.axvline(0.8, ls="--", color=GRAY, alpha=0.5, lw=1, label="0.80 baseline")
...
ax2.set_title("Confusion Matrix", fontweight="bold", color=BLUE, fontsize=12)
ax2.set_ylabel("Actual", fontsize=10)
ax2.set_xlabel("Predicted", fontsize=10)

ax3 = fig.add_subplot(gs[0, 2])
fpr, tpr, _ = roc_curve(y_test, y_proba)
ax3.plot(fpr, tpr, color=CORAL, lw=2.5, label=f"XGBoost (AUC = {auc:.3f})")
ax3.plot([0,1],[0,1], ls="--", color=GRAY, lw=1.2, label="Random Classifier")
ax3.fill_between(fpr, tpr, alpha=0.08, color=CORAL)
ax3.set_xlabel("False Positive Rate", fontsize=10)
ax3.set_ylabel("True Positive Rate", fontsize=10)
ax3.set_title("ROC Curve", fontweight="bold", color=BLUE, fontsize=12)
...
colors_fi = [CORAL if v > feat_imp.median() else BLUE for v in feat_imp.values]
bars_fi = ax4.barh(feat_imp.index, feat_imp.values,
...
ax4.set_title("XGBoost Feature Importance", fontweight="bold", color=BLUE, fontsize=12)
ax4.set_xlabel("Importance Score", fontsize=10)
ax4.spines[["top","right"]].set_visible(False)
ax4.legend(handles=[mpatches.Patch(color=CORAL, label="High importance"),
                     mpatches.Patch(color=BLUE, label="Lower importance")], fontsize=9)

ax5 = fig.add_subplot(gs[1, 2])
for outcome, color, label in [(0, BLUE,"Non-Diabetic"),(1, CORAL,"Diabetic")]:
    ax5.hist(y_proba[y_test.values==outcome], bins=25, alpha=0.65,
             color=color, label=label, edgecolor="white")
ax5.axvline(0.5, ls="--", color=GRAY, lw=1.5, label="Decision threshold")
ax5.set_xlabel("Predicted Probability", fontsize=10)
ax5.set_ylabel("Count", fontsize=10)
ax5.set_title("Probability Distribution", fontweight="bold", color=BLUE, fontsize=12)
ax5.legend(fontsize=9)
ax5.spines[["top","right"]].set_visible(False)

fig.suptitle("XGBoost Diabetes Prediction - Model Evaluation Dashboard",
             fontsize=16, fontweight="bold", color=BLUE, y=1.01)
plt.savefig(f"{OUTPUT_DIR}/fig4_evaluation.png", bbox_inches="tight", facecolor="white")
plt.show()
print("Figure 4 saved: Evaluation dashboard")

fig, axes = plt.subplots(1, 3, figsize=(16, 5))
fig.suptitle("Key EDA Insights", fontsize=14, fontweight="bold", color=BLUE)

age_labels = ["<30","30-44","45-59","60+"]
df_fe["Age_Label"] = df_fe["Age_Group"].map(dict(enumerate(age_labels)))
grp_data = df_fe.groupby("Age_Label")["Outcome"].mean().reindex(age_labels)
axes[0].bar(grp_data.index, grp_data.values*100, color=[BLUE,TEAL,CORAL,GREEN], alpha=0.85)
axes[0].set_title("Diabetes Rate by Age Group", fontweight="bold", color=BLUE)
axes[0].set_ylabel("Diabetic (%)")
axes[0].spines[["top","right"]].set_visible(False)
for i, v in enumerate(grp_data.values):
    axes[0].text(i, v*100+0.5, f"{v*100:.1f}%", ha="center", fontsize=9, fontweight="bold")

bmi_labels = ["Underweight","Normal","Overweight","Obese"]
df_fe["BMI_Label"] = df_fe["BMI_Category"].map(dict(enumerate(bmi_labels)))
bmi_data = df_fe.groupby("BMI_Label")["Outcome"].mean().reindex(bmi_labels)
axes[1].bar(bmi_data.index, bmi_data.values*100, color=[BLUE,TEAL,CORAL,GREEN], alpha=0.85)
axes[1].set_title("Diabetes Rate by BMI Category", fontweight="bold", color=BLUE)
axes[1].set_ylabel("Diabetic (%)")
axes[1].spines[["top","right"]].set_visible(False)
axes[1].tick_params(axis="x", rotation=15)
for i, v in enumerate(bmi_data.values):
    axes[1].text(i, v*100+0.5, f"{v*100:.1f}%", ha="center", fontsize=9, fontweight="bold")

gluc_labels = ["Normal (<100)","Pre-diabetic","Diabetic range"]
df_fe["Gluc_Label"] = df_fe["Glucose_Category"].map(dict(enumerate(gluc_labels)))
gluc_data = df_fe.groupby("Gluc_Label")["Outcome"].mean().reindex(gluc_labels)
axes[2].bar(gluc_data.index, gluc_data.values*100, color=[GREEN,TEAL,CORAL], alpha=0.85)
axes[2].set_title("Diabetes Rate by Glucose Category", fontweight="bold", color=BLUE)
axes[2].set_ylabel("Diabetic (%)")
axes[2].spines[["top","right"]].set_visible(False)
for i, v in enumerate(gluc_data.values):
    axes[2].text(i, v*100+0.5, f"{v*100:.1f}%", ha="center", fontsize=9, fontweight="bold")

plt.tight_layout()
plt.savefig(f"{OUTPUT_DIR}/fig5_eda_insights.png", bbox_inches="tight", facecolor="white")
plt.show()
print("Figure 5 saved: EDA insights")
...
def predict_diabetes(patient_dict, model, scaler, feature_names):
    d = patient_dict.copy()
    bmi, age, gluc, ins = d["BMI"], d["Age"], d["Glucose"], d["Insulin"]
    d["BMI_Category"]     = bmi_cat(bmi)
    d["Age_Group"]        = age_grp(age)
    d["Glucose_Category"] = gluc_cat(gluc)
    d["Glucose_BMI"]      = gluc * bmi
    d["Age_BMI"]          = age * bmi
    d["Insulin_Glucose"]  = ins / (gluc + 1)
    row = pd.DataFrame([d])[feature_names]
    row_sc = scaler.transform(row)
    prob = model.predict_proba(row_sc)[0][1]
    risk = "High" if prob >= 0.6 else "Moderate" if prob >= 0.4 else "Low"
    return prob, risk

patient = {
    "Pregnancies": 1, "Glucose": 190, "BloodPressure": 70,
    "SkinThickness": 30, "Insulin": 90, "BMI": 34.0,
    "DiabetesPedigreeFunction": 0.6, "Age": 50
}

with open(f"{OUTPUT_DIR}/feature_names.json", "r") as f:
    feature_names = json.load(f)

prob, risk = predict_diabetes(patient, model, scaler, feature_names)
...
try:
    ...
    plt.title("SHAP Feature Impact - Why Did the Model Predict Diabetes?",
              fontsize=13, fontweight="bold", color=BLUE, pad=12)
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/fig6_shap_summary.png", bbox_inches="tight", facecolor="white")
    plt.show()
    print("Figure 6 saved: SHAP summary plot")

    plt.figure(figsize=(10, 6))
    shap.summary_plot(shap_values, X_test, plot_type="bar", show=False)
    plt.title("SHAP Mean Absolute Impact - Global Feature Importance",
              fontsize=13, fontweight="bold", color=BLUE, pad=12)
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}/fig7_shap_bar.png", bbox_inches="tight", facecolor="white")
    plt.show()
    print("Figure 7 saved: SHAP bar plot")
